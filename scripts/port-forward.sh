...
echo -e "${BLUE}Setting up port forwarding to Airflow webserver...${NC}"
echo ""
...
# Get webserver service name
WEBSERVER_SERVICE="${RELEASE_NAME}-webserver"
if ! kubectl get svc "$WEBSERVER_SERVICE" -n "$NAMESPACE" &> /dev/null; then
    echo -e "${RED}Error: Service '$WEBSERVER_SERVICE' not found in namespace '$NAMESPACE'${NC}"
...
echo -e "${BLUE}Forwarding local port $LOCAL_PORT to Airflow webserver...${NC}"
